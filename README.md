...
برای پیاده‌سازی وضعیت سیستم انژری در ساختمان از الگوی State استفاده کردیم. در کلاس BuildingManager یک فیلد به نام systemState وجود دارد که از نوع SystemState است. کلاس SystemState می‌تواند پیاده‌سازی‌های مختلفی داشته باشد. این کلاس یک تابع دارد که بر اساس آن وضعیت، لیست سیستم‌هایی که باید روشن باشند برگردانده می‌شود. مثلا در حالت اقتصادی فقط سیستم‌های حیاتی باید روشن باشند. کلاس SystemState می‌تواند پیاده‌سازی‌های متفاوتی داشته باشد که در اینجا طبق گزارش کار سه حالت فعال، اقتصادی و خاموش پیاده‌سازی شده‌اند. با استفاده از این الگو برای تغییر وضعیت سیستم انرژی، تنها کافی است مقدار فیلد systemState در BuildingManager تغییر کند.

همچنین برای پیاده‌سازی سیاست‌های متفاوت هزینه انرژی از الگوی Strategy استفاده کردیم. اینترفیس PricePoly نشانگر یک استراتژی مختلف برای محاسبه هزینه است. این اینترفیس پیاده‌سازی‌های مختلفی می‌تواند داشته باشد. در کلاس BuildingManager یک فیلد PricePolicy وجود دارد که استراتژی محاسبه هزینه را مشخص می‌کند و هزینه توسط آن فیلد محاسبه می‌شود. لذا برای تغییر استراتژی محاسبه هزینه تنها کافی است این فیلد را تغییر دهیم و تغییر دیگری لازم نیست.

برای پیاده‌سازی پروژه نیز از رویکرد TDD استفاده کردیم. در ابتدا تست‌هایی برای سناریوهایی مختلف برای کلاس BuildingSystem نوشتیم. همچنین متدهای پابلیک این کلاس را بدون پیاده‌سازی طراحی کردیم. در نهایت این کلاس را طوری کامل کردیم تا تست‌ها پاس شوند.
...
در فایل SymbolTable.java مقادیر 0 و 1 را با ثابت های TRUE_VALUE و FALSE_VALUE جایگزین کردیم.
...
الگو های state و strategy جزو الگو های رفتاری (behavioral) هستند.

## 3- الگو های مناسب برای تغییر حالت و سیاست

همانطور که در متن آزمایش هم پیشنهاد شده، الگو های مناسب state و strategy هستند.
state برای مدیریت حالت فعلی سیستم و startegy برای تغییر سیاست محاسبۀ هزینه.
استفاده از این الگو ها کد ما را ساختار مند تر میکند وگرنه مثلا برای دستیابی به هدف مشابه، باید از ساختار شرطی طولانی استفاده کنیم و در هر کدام از شاخه ها موارد مربوط به آن حالت را قرار دهیم. در حالی که با استفاده از الگوی state تمام موارد مربوط به هر حالت را در کلاس آن قرار دادیم و نظم بسیار بیشتری ایجاد کردیم.
پس مثلا برای وضعیت سیستم یک کلاس اصلی SystemState داریم که هر کدام از سه کلاس وضعیتمان از آن ارث بری میکنند و در هر کدام از آنها لیست دیوایس های فعال مشخص میشود.
در مورد محاسبه هزینه هم که الگوی startegy را پیاده میکنیم، یک اینترفیس به نام pricepolicy ساختیم که هر سه کلاسی که برای سه استراتژی محاسبۀ هزینه ساختیم آن را پیاده میکنند و هر کدام ضریب مشخصی را به انرژی مصرفی برای محاسبۀ هزینه اعمال میکنند.
...
8.  technical debt: در فایل Phase1CodeGenerator بیست todo وجود دارد که حجم زیادی بدهی فنی ایجاد کرده.
9.  feature envy: همینطور که در todo های فایل phase1codegenerator هم نوشته شده، بسیاری از توابع آن باید به کلاس Complete class منتقل شوند چون دارند با دیتای آن کار میکنند.
10. long parameter list: متد addToTable در فایل MethodOverloader چهار پارامتر میگیرد که میتواند نشانه ای از این کد اسمل باشد.
